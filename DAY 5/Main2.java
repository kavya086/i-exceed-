...
class Main extends Thread{
    ...
    Main() throws Exception //for .join() since it interrupts executing both the threads
    {
        this.start();//calls run()
        this.join(); // finishes the execution of one thread and allows the next thread to enter the critical state only after the execution of the first thread
     }
    public void run() 
    {
        try
